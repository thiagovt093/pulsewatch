...
export default function DashboardPage() {
  ...
  useEffect(() => {
    monitoringService.getApis().then((data) => {
      setApis(data);
      setLoading(false);
    });
  }, []);
  ...
  const stats = [
    { label: "Total de APIs", value: apis.length, icon: WifiHighIcon, color: "orange" },
    { label: "Operacionais", value: up, icon: CheckCircleIcon, color: "green" },
    { label: "Com falha", value: down, icon: XCircleIcon, color: "red" },
    { label: "Degradadas", value: degraded, icon: WarningIcon, color: "yellow" },
  ];
            ...
            >
              <Card className="rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-xl">
                <CardContent className="flex items-center gap-4 p-5">
                  <div className={`rounded-xl border p-3 ${colors[stat.color]}`}>
